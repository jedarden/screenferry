...
| **I6a** | **Block-layer** working set MUST stay ≤ 1 MB regardless of file size (~660 KB per GE context at the adopted design: 72 KB matrix + 192 KB block + 192 KB recover() buffer + 6 KB degree table + 3 KB scratch, totaling ~1.32 MB for payload + manifest contexts, still under the 1 MB limit per I6a definition of steady-state working set). **Note:** The earlier "264 KB" figure understated the working set by 2.5×; the correct figure is 660 KB. | A5 headless block-layer memory assertion |
...
| Coding | **Fixed K=768 multi-block stream** — `blockCount_manifest = ceil(blockCount × blockHashLen / blockSize)`, same LT encoder, same GE decoder. Each manifest block is fountain-coded independently with K=768, inheriting the flat-cost property. Uses `blockHashLen` (not a hardcoded 4) for correctness with any hash length. |
...
encoding is human-typable and avoids ambiguous characters (no I/L/O/U). The term "base32" here
specifically means Crockford base32, not generic base32 or base36.
...
| **A4** | Lossy channel | A1 setup; camera deliberately occluded **10%** of the time in 2-second bursts (e.g., 2 seconds every 20 seconds). Tests the fountain code's tolerance to moderate, predictable loss. 10% occlusion on top of baseline erasure keeps total erasure below e_max (34.9%) in typical conditions, ensuring the transfer can complete while demonstrating the dwell margin's purpose. | Send 1 MB | Byte-identical; completes in ≤ 2.0× the A1 frame count | Does not complete in 3.0× A1 frame count (tests e_max cliff) |

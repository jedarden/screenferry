...
| 2026-08-02 | **Local-diagnostics rule added** — T8 added to threat model: local logging (console logs, window properties, error messages) MUST NEVER expose payload bytes (file content). Metadata (filenames, sizes, performance metrics) is permissible for debugging. All existing code (RecvSession.stats, bf-5vm's stall detector, window.sfStats, clipboard repair, storage/export logging) audited and confirmed compliant. | `docs/notes/bf-1zxy-local-diagnostics-rule.md` |
...
| **T8** | **Local logging must never expose payload bytes.** Console logs, window properties, and error messages MUST NOT log file content (payload bytes). Metadata (filenames, sizes, performance metrics) is permissible for debugging. | **Committed.** Aligns privacy posture across all channels: network telemetry is forbidden (T7) and local diagnostics must respect the same boundary. Prevents shoulder-surfing, device theft, and browser extension access to console logs from becoming a privacy hole. | Code review: audit console.log statements for Uint8Array/ArrayBuffer; assert no payload data in window properties; extend §14.4 network assertion to audit console output |

...
| E2 | **Repetition is derived per-block, not signalled** | Files smaller than one fragment (< 256 B) pad to one fragment (`K = 1`). The research's `K < 8` guard is adopted as `MIN_LT_K = 8`: below 8 fragments, the LT fountain code behaves badly, so the encoder sends plain repetition instead. Repetition is a **PER-BLOCK property** derived from `blockK(blockIndex, blockSize, fileSize) < MIN_LT_K` (E3a's shared function). Both encoder and decoder compute it independently — no signalling required. A beacon flag would force repetition on all 21,845 blocks or none, which is wrong; the last block of any file may have K=1-7, so repetition applies automatically wherever K is small. |
